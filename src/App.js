...
const T={bg:"#0f0f12",surface:"#1a1a22",surfaceAlt:"#22222e",border:"#2a2a38",primary:"#4f8cff",accent:"#ff6b4a",green:"#4ade80",yellow:"#fbbf24",text:"#e8e8f0",textDim:"#8888a0",textFaint:"#55556a",pink:"#f472b6",purple:"#a78bfa",font:`'Segoe UI',-apple-system,system-ui,sans-serif`};
...
const GRADS={
  outdoor:[
    "linear-gradient(135deg,#0f2b1f 0%,#1a5c3a 40%,#2d8a56 80%,#3aad6a 100%)",
    "linear-gradient(135deg,#0d2e22 0%,#166040 40%,#28906a 80%,#34b87a 100%)",
    "linear-gradient(135deg,#122e1e 0%,#1e6b40 40%,#36985e 80%,#44c07a 100%)",
  ],
  food:[
    "linear-gradient(135deg,#2e1a0d 0%,#6a3a18 40%,#b06a2e 80%,#d4903a 100%)",
    "linear-gradient(135deg,#321810 0%,#7a3520 40%,#c06030 80%,#e08040 100%)",
    "linear-gradient(135deg,#2a1c10 0%,#5a3a1a 40%,#9a6a30 80%,#c09040 100%)",
  ],
  adventure:[
    "linear-gradient(135deg,#1e0d2e 0%,#4a1d5a 40%,#8a3aa0 80%,#b050c8 100%)",
    "linear-gradient(135deg,#160d3a 0%,#3a1d6a 40%,#7040a8 80%,#9558cc 100%)",
    "linear-gradient(135deg,#220d28 0%,#5a1d58 40%,#9a3a90 80%,#c050b0 100%)",
  ],
  creative:[
    "linear-gradient(135deg,#0d1a30 0%,#1a3a6a 40%,#3060a8 80%,#4080cc 100%)",
    "linear-gradient(135deg,#0d1e38 0%,#1a4478 40%,#2a68b0 80%,#3a8ad4 100%)",
    "linear-gradient(135deg,#101a3a 0%,#2a3070 40%,#4050a8 80%,#5a70d0 100%)",
  ],
  nightlife:[
    "linear-gradient(135deg,#160d28 0%,#381a58 40%,#6a30a0 80%,#8a48c0 100%)",
    "linear-gradient(135deg,#1a0d30 0%,#441a68 40%,#7a30b0 80%,#9a48d0 100%)",
    "linear-gradient(135deg,#120d24 0%,#301a4a 40%,#582890 80%,#7040b0 100%)",
  ],
  chill:[
    "linear-gradient(135deg,#0d1820 0%,#1a3548 40%,#2a5878 80%,#3878a8 100%)",
    "linear-gradient(135deg,#0d1422 0%,#182e44 40%,#285070 80%,#3870a0 100%)",
    "linear-gradient(135deg,#0f1a28 0%,#1e3858 40%,#306088 80%,#4080b8 100%)",
  ],
  romantic:[
    "linear-gradient(135deg,#2e0d18 0%,#6a1a38 40%,#a83060 80%,#d04878 100%)",
    "linear-gradient(135deg,#380d1e 0%,#7a1a3e 40%,#b83060 80%,#e04878 100%)",
    "linear-gradient(135deg,#260d16 0%,#5a1a34 40%,#903058 80%,#b84070 100%)",
  ],
  meaningful:[
    "linear-gradient(135deg,#0d2222 0%,#1a4a42 40%,#2a7a6a 80%,#38a890 100%)",
    "linear-gradient(135deg,#0d2828 0%,#1a5548 40%,#2a8870 80%,#38b898 100%)",
    "linear-gradient(135deg,#102220 0%,#204a3a 40%,#307860 80%,#40a880 100%)",
  ],
};
const getGrad = (date) => {
...
const CAT_ACCENT={outdoor:"#4ade80",food:"#fbbf24",adventure:"#e879f9",creative:"#60a5fa",nightlife:"#c084fc",chill:"#7dd3fc",romantic:"#fb7185",meaningful:"#5eead4"};
...
function MysteryInvite({ date, scheduledFor, onClose, onSend }) {
  ...
  const [desc, setDesc] = useState(`Dately ✨\n\n${dressHint}\n\n${herring}\n\nThat's all you get. See you there 😏`);

  const send = () => {
    const ics = generateICS("Dately ✨", desc, scheduledFor, time);
    const blob = new Blob([ics], { type: "text/calendar" });
    const url = URL.createObjectURL(blob);
    const subject = encodeURIComponent("You've got a Dately 😏");
    const body = encodeURIComponent(desc + "\n\n(Calendar invite attached — check your downloads!)");
    const a = document.createElement("a");
    a.href = url; a.download = "dately.ics"; a.click();
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, padding: 20, overflowY: "auto" }}>
      <div style={{ ...crd(), maxWidth: 440, width: "100%", padding: 30 }}>
        <h3 style={{ color: T.text, fontSize: 20, margin: "0 0 4px", fontWeight: 700 }}>📅 Schedule Date</h3>
      ...
      <div style={{ ...crd(), maxWidth: 440, width: "100%", padding: 0, maxHeight: "80vh", overflowY: "auto" }}>
        <div style={{ padding: "20px 24px 16px", borderBottom: `1px solid ${T.border}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <h3 style={{ color: T.text, fontSize: 18, margin: 0, fontWeight: 700 }}>💡 Build the Anticipation</h3>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, padding: 20 }}>
      <div style={{ ...crd(), maxWidth: 460, width: "100%", padding: 30, maxHeight: "90vh", overflowY: "auto" }}>
        <h3 style={{ color: T.text, fontSize: 20, margin: "0 0 4px", fontWeight: 700 }}>How'd It Go?</h3>
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 999 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 600, maxHeight: "92vh", overflowY: "auto", background: T.bg, borderRadius: "20px 20px 0 0" }}>
        <div style={{ background: grad, padding: "32px 24px 28px", position: "relative", overflow: "hidden", borderRadius: "20px 20px 0 0" }}>
          ...
          <h2 style={{ color: "#fff", fontSize: 26, margin: "0 0 10px", fontWeight: 800, fontFamily: T.font, textShadow: "0 2px 8px rgba(0,0,0,0.3)", lineHeight: 1.2 }}>{d.title}</h2>
...
function Card({ date, onClick, grid }) {
  ...
  return (
    <div onClick={onClick} style={{ flex: grid ? undefined : "0 0 180px", minWidth: grid ? undefined : 180, height: grid ? 210 : 240, borderRadius: 18, padding: 0, background: grad, cursor: "pointer", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden", scrollSnapAlign: "start", boxShadow: "0 4px 24px rgba(0,0,0,0.35)", border: "1px solid rgba(255,255,255,0.08)" }}>
      {/* Right accent bar */}
      <div style={{ position: "absolute", top: 0, right: 0, width: 4, height: "100%", background: `linear-gradient(180deg, ${accent}00 0%, ${accent} 30%, ${accent} 70%, ${accent}00 100%)`, borderRadius: "0 18px 18px 0" }} />
      {/* Decorative glow orbs */}
      <div style={{ position: "absolute", top: -40, right: -40, width: grid ? 110 : 130, height: grid ? 110 : 130, borderRadius: "50%", background: `radial-gradient(circle, ${accent}12 0%, transparent 70%)` }} />
      <div style={{ position: "absolute", bottom: -30, left: -30, width: 90, height: 90, borderRadius: "50%", background: `radial-gradient(circle, ${mood.color}08 0%, transparent 70%)` }} />
      {/* Top bar */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: grid ? "14px 14px 0" : "16px 16px 0" }}>
        <div style={{ width: grid ? 38 : 44, height: grid ? 38 : 44, borderRadius: 12, background: "rgba(255,255,255,0.08)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: grid ? 18 : 21, border: "1px solid rgba(255,255,255,0.1)", boxShadow: "0 2px 8px rgba(0,0,0,0.2)" }}>{emoji}</div>
        <div style={{ background: "rgba(0,0,0,0.25)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", padding: "6px 12px", borderRadius: 10, textAlign: "center", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 2px 8px rgba(0,0,0,0.2)" }}>
          <span style={{ color: tier.color, fontSize: 13, fontWeight: 800, fontFamily: T.font, display: "block", lineHeight: 1.1 }}>${date.budget}</span>
          <span style={{ color: tier.color, fontSize: 7, fontWeight: 700, fontFamily: T.font, letterSpacing: 0.8, opacity: 0.8, textTransform: "uppercase" }}>{tier.tag}</span>
        </div>
      </div>
      {/* Content */}
      <div style={{ marginTop: "auto", padding: grid ? "0 14px 14px" : "0 16px 16px" }}>
        {/* Mood pill */}
        <div style={{ marginBottom: 6 }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 4, background: `${mood.color}18`, border: `1px solid ${mood.color}25`, borderRadius: 6, padding: "3px 8px", fontSize: 9, fontWeight: 700, color: mood.color, fontFamily: T.font, letterSpacing: 0.5, textTransform: "uppercase" }}>{mood.emoji} {mood.label}</span>
        </div>
        <p style={{ color: "#fff", fontSize: grid ? 14 : 15, fontWeight: 700, margin: "0 0 8px", lineHeight: 1.25, fontFamily: T.font, textShadow: "0 2px 8px rgba(0,0,0,0.5)", letterSpacing: -0.2 }}>{date.title}</p>
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
          {date.vibe.slice(0, grid ? 2 : 3).map(v => <span key={v} style={{ background: "rgba(255,255,255,0.08)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", color: "rgba(255,255,255,0.6)", fontSize: grid ? 9 : 10, fontWeight: 600, padding: "3px 8px", borderRadius: 6, fontFamily: T.font, border: "1px solid rgba(255,255,255,0.08)", letterSpacing: 0.2 }}>{v}</span>)}
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: T.bg, fontFamily: T.font }}>
      {swipeMode && <div style={{ position: "fixed", inset: 0, background: T.bg, zIndex: 900, display: "flex", flexDirection: "column", fontFamily: T.font }}>
        ...
        {swipeIdx < swipeDeck.length ? (() => {
          ...
          return <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 20px", position: "relative", overflow: "hidden" }}>
            ...
            >
              ...
              <div style={{ padding: "30px 24px", display: "flex", flexDirection: "column", height: "100%", boxSizing: "border-box" }}>
                ...
                <h2 style={{ color: "#fff", fontSize: 28, fontWeight: 800, margin: "0 0 12px", lineHeight: 1.2, textShadow: "0 2px 12px rgba(0,0,0,0.5)" }}>{d.title}</h2>
          ...
          <h2 style={{ color: T.text, fontSize: 22, fontWeight: 700, margin: "0 0 8px" }}>You've seen them all!</h2>
      ...
      <div style={{ background: T.surface, borderBottom: `1px solid ${T.border}`, padding: "env(safe-area-inset-top, 0) 20px 0", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 0" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}><span style={{ fontSize: 22 }}>🔥</span><h1 style={{ color: T.text, fontSize: 18, margin: 0, fontWeight: 800 }}>Dately</h1></div>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "20px 16px 100px" }}>
        ...
        {tab === "calendar" && <>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
            <h2 style={{ color: T.text, fontSize: 20, margin: 0, fontWeight: 700 }}>Upcoming Dates</h2>
        ...
        {tab === "library" && <>
          {/* Filters at top */}
          <h2 style={{ color: T.text, fontSize: 20, margin: "0 0 14px", fontWeight: 700 }}>Date Library</h2>
          ...
          {rows.filter(row => row.dates.filter(d => { if (bf !== null && d.budget > bf) return false; if (cf && d.category !== cf) return false; return true; }).length > 0).map(row => <div key={row.label} style={{ marginBottom: 24 }}>
            <h3 style={{ color: T.text, fontSize: 17, margin: "0 0 12px", fontWeight: 700 }}>{row.label}</h3>
            <div style={{ display: "flex", gap: 14, overflowX: "auto", paddingBottom: 8, scrollSnapType: "x mandatory", WebkitOverflowScrolling: "touch" }}>
          ...
          <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 24 }}>
            <h3 style={{ color: T.text, fontSize: 17, margin: "0 0 8px", fontWeight: 700 }}>All Dates</h3>
        ...
        {tab === "memories" && <>
          <h2 style={{ color: T.text, fontSize: 20, margin: "0 0 18px", fontWeight: 700 }}>Date Memories</h2>
    ...
    <div style={{ minHeight: "100vh", background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: T.font, padding: 20 }}>
      <div style={{ ...crd(), maxWidth: 420, width: "100%", padding: 40 }}>
        <div style={{ textAlign: "center", marginBottom: 30 }}>
          ...
          <h1 style={{ color: T.text, fontSize: 34, margin: 0, fontWeight: 800, letterSpacing: "-1px" }}>Dately</h1>
          <p style={{ color: T.textDim, margin: "10px 0 0", fontSize: 15, lineHeight: 1.5 }}>Never run out of date ideas again.<br />90+ curated experiences for every budget.</p>
        </div>
...
function Splash({ onDone }) {
  ...
  return (
    <div style={{
      ...
      <style>{`
        @keyframes fireGlow {
          0%, 100% { filter: drop-shadow(0 0 8px #ff6b4a55) drop-shadow(0 0 25px #ff6b4a22); transform: scale(1) translateY(0); }
          50% { filter: drop-shadow(0 0 20px #ff6b4a88) drop-shadow(0 0 50px #ff6b4a44); transform: scale(1.08) translateY(-4px); }
        }
        @keyframes logoReveal {
          0% { opacity: 0; transform: translateY(16px) scale(0.92); letter-spacing: 12px; }
      ...
      <div style={{
        ...
        background: "radial-gradient(circle, #ff6b4a18 0%, transparent 70%)",
        animation: phase >= 1 ? "pulseRing 2s ease-in-out infinite" : "none",
        opacity: phase >= 1 ? 1 : 0,
      }} />
      ...
      <div style={{
        ...
        opacity: phase >= 1 ? 1 : 0,
        animation: phase >= 1 ? "fireGlow 2s ease-in-out infinite" : "none",
        transition: "opacity 0.5s ease",
      }}>🔥</div>

      {/* DATELY text */}
      <h1 style={{
        color: "#fff", fontSize: 48, fontWeight: 900, margin: "0 0 16px",
        opacity: phase >= 1 ? 1 : 0,
        animation: phase >= 1 ? "logoReveal 0.8s cubic-bezier(0.16,1,0.3,1) forwards" : "none",
        background: `linear-gradient(135deg, ${T.text} 0%, ${T.accent} 50%, ${T.primary} 100%)`,
      ...
      {/* Tagline */}
      <p style={{
        color: T.textDim, fontSize: 17, fontWeight: 500, margin: 0,
        opacity: phase >= 2 ? 1 : 0,
      ...
      {/* Subtle loading bar */}
      <div style={{
        position: "absolute", bottom: 60, width: 120, height: 3,
        background: T.border, borderRadius: 2, overflow: "hidden",
        opacity: phase >= 1 ? 0.6 : 0, transition: "opacity 0.3s",
      }}>
        <div style={{
